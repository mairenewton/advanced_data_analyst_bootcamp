...
dimension: traffic_source_comparitor {
  type: string
  sql: CASE
  WHEN {% condition choose_a_source %}
         ${traffic_source}
         {% endcondition %}
    THEN ${traffic_source}
  ELSE 'All Other Source'
  END ;;
}

  dimension: traffic_source_filter {
    type: yesno
    sql: {% condition choose_a_source %}
               ${traffic_source}
               {% endcondition %}
          ;;
  }

filter: choose_a_source {
  type: string
...
measure: count_users_traffic_source {
  type: count
  filters: {
...
measure: user_percentage {
  type: number
  sql: 1.0*${count_users_traffic_source}/ nullif(${count},0);;
}

dimension: email {
  type: string
  required_access_grants: [pii_information]
  sql: ${TABLE}.email ;;
  link: {
    label: "Dashboard"
